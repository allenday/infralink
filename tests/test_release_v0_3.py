...
SCRIPT = PROJECT_ROOT / "scripts" / "release_v0_3.py"
SOURCE_SHA = "a" * 40
...
def load_module():
    spec = importlib.util.spec_from_file_location("release_v0_3", SCRIPT)
...
def test_validate_release_inputs_accepts_only_v0_3_0_and_main_sha() -> None:
    module = load_module()
...
def test_write_checksums_is_canonical_and_asset_discovery_is_exact(tmp_path: Path) -> None:
    module = load_module()
    wheel = tmp_path / "infralink-0.3.0-py3-none-any.whl"
    sdist = tmp_path / "infralink-0.3.0.tar.gz"
...
def test_write_checksums_rejects_nonexact_package_set(
    ...
    module = load_module()
    (tmp_path / "infralink-0.3.0-py3-none-any.whl").write_bytes(b"wheel")
    if extra_name is not None:
        (tmp_path / "infralink-0.3.0.tar.gz").write_bytes(b"sdist")
